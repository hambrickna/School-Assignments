...
def initialize_pop(example_genome: str, pop_size: int) -> Population:
    ...
    keyCharacters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ;'/=[]-,."
    population = []
    for i in range(pop_size):
        genome = ""
        copyKeyChars = keyCharacters
        for j in range(len(example_genome)):
            char = random.choice(copyKeyChars)
            genome = genome + char
            copyKeyChars.replace(char, "")

        individual = initialize_individual(genome, 0)
...
def recombine_pair(parent1: Individual, parent2: Individual) -> Population:
    ...
    choice = random.choice(range(len(parent1["genome"])))
    cross_string = parent1["genome"][choice:]
    cross_stringTwo = parent2["genome"][choice:]
    for i in range(len(cross_string):
        if cross_stringTwo[i] not in parent1["genome"][:choice]:
            cross_string.replace(cross_string[i], cross_stringTwo[i])
            cross_string.replace(cross_stringTwo[i], cross_string[i])
